...
### Naming convention for Transaction controller
* NOTE: It is to better place all http requests under TRANSACTION CONTROLLER.

* If we hitting endpoint url /v1/signup/registration/generate-challenge of Sign Up services and  url is present in the first thread group , then the naming convention should be SignUpService_S01_T01_GenerateChallenge (SignUpService ---> Module name, S01 ---> First thread group name, T01 ---> Transaction number, GenerateChallenge ---> Name of endpoint url
	*SignUpService ---> Module name
	*S01 ---> First thread group name
	*T01 ---> Transaction number
	*GenerateChallenge ---> Name of endpoint url
	

### Naming convention for Variable names
* Values declared in User defined config should be named as u_variablename.
	* eg: ServerIP ---> u_serverip
	
* Values fetched from parameter file or csv file should be named as p01_variablename.
	* identifier  ---> p01_identifier
	* if there are multiple files like file1 , file2 , file3.
		* file1 ---> p01_identifier,p01_encodeddata
		* file2 ---> p02_transactionid
		* file3 ---> p03_token, p03_phonenum
		
* Correlated values (dynamic values) captured from response should be stored in variable as   c_transactionid
	* transactionid  ---> c_transactionid
	
* Any values generated by Beanshell or JSR223 processors need to be named as g_utctime
	* UTCtime ---> g_utctime
	
### Naming convention for csv files
* If Testdata for script one and for testcase present in threadgroup S01 and data is passed in particular request then you can name csv file as SignUpService_S02_T01_VerifyChallenge.csv

### Usage of Ultimate Thread group for performance testing
* For performance test execution is better to use Ultimate thread group. we can use it for multi purpose like for scalability testing

* In this thread group we can customise starting number of users, ramp up time and hold time. Again additional users can be ramped up again by providing initial delay.
...
* If we are performing load test with 100TPS as hits / sec in one thread group. Then we need to provide value hits / minute as in Constant Throughput Timer
	* Value = 100 X 60
			= 6000
...
* Dropdown option in Constant Throughput Timer
	...
	* Calculate Throughput based on as = this thread
		* If we are performing scalability testing we need to calculate throughput for 100 TPS as 
          Value = (100 * 60 )/(Number of users)

### Usage of bzm - Weighted Switch Controller for the distribution of hits volume with respect to percentages in a single thread group
* This controller helps in distributing hits among the different end point urls. We can provide percentage of total hits for the specific url. 

### Usage of bzm - Random CSV Data Set Configuration
* This config helps to pass random parameter values to simulate real world scenario for performance testing.