...
//		String[] activityArray = { "UPLOAD_PACKET", "VALIDATE_PACKET", "QUALITY_CHECK", "OSI_VALIDATE",
...
//		computeAverageDifference(regIdsMap, activities);
	...
	private void obtainActivitiesMappedByRegistrationIds(Session session, List<String> regidList) {
		/*
		 * key for the outer map is registration ID, key for the inner map is the
		 * activity name.
		 */
		Map<String, Map<String, Object>> transactionTimesByRegIds = new LinkedHashMap<>();
		int c = 0;
		List<TransactionEntity> trList;
		// List<TransactionEntity> trList = dbAccess.getRegTransactionForRegId(session,
		// regidList);
...
//			Iterator<Entry<String, Long>> iterator = activitySet.iterator();
	...
	private String getCrTimeStampOfFirstStage(String regid, Session session) {
		String crTimestamp = "";
		String transactionName = "PACKET_RECEIVER";
		crTimestamp = dbAccess.obtainCrDTimeOfTransactionForRegid(session, regid, transactionName);
		return crTimestamp;

	}

	private Map<String, Object> obtainTimeSpentTransactionWise(List<TransactionEntity> trList, Session session) {
		...
		Double totalTimeSpentSec = (double) (totalTimeSpentMillis / 1000);
	...
	private Double computeTotalSeconds(Map<String, Object> timeSpentTrMap) {
		...
		for (Map.Entry<String, Object> entry : timeSpentTrMap.entrySet()) {
			if (entry.getKey().equalsIgnoreCase("CR_DTIME"))
				continue;
