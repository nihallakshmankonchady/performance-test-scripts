...
public class DBAccessImpl {
	...
	public String obtainCrDTimeOfTransactionForRegid(Session session, String regid, String transactionName) {
		java.time.LocalDateTime crTimestamp = null;
		String query = "select trEntity.createDateTime from TransactionEntity trEntity where reg_id='" + regid
				+ "' and trEntity.trntypecode = '" + transactionName + "'";
		Query q = session.createQuery(query);
		List<java.time.LocalDateTime> list = q.getResultList();
		try {
			crTimestamp = list.get(0);
		} catch (Exception e) {
			return "";
		} 
		return crTimestamp.toString();
	}
